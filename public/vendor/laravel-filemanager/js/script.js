...
$('#multi_selection_toggle').click(function () {
    ...
    $('#multi_selection_toggle i').toggleClass('fa-times', multi_selection_enabled).toggleClass('fa-check-double', !multi_selection_enabled);
...
function setOpenFolders() {
    $('#tree [data-path]').each(function (index, folder) {
        ...
        $(folder).children('i').toggleClass('fa-folder-open', should_open).toggleClass('fa-folder', !should_open);
    });
...
function generatePaginationHTML(el, args) {
    ...
    if (args.pageRange === null) {
        for (i = 1; i <= totalPage; i++) {
            var button = $(template).attr('data-num', i).append($(linkTemplate).html(i));
            if (i == currentPage) {
    ...
    if (rangeStart <= 3) {
        ...
        var button = $(template).attr('data-num', 1).append($(linkTemplate).html(1));
        el.append(button);

        var button = $(template).addClass('disabled').append($(linkTemplate).html('...'));
        el.append(button);
    }
    ...
    if (rangeEnd >= totalPage - 2) {
        ...
        var button = $(template).attr('data-num', totalPage).append($(linkTemplate).html(totalPage));
        el.append(button);
    }
}
...
function loadItems(page) {
    loading(true);
    performLfmRequest('jsonitems', { show_list: show_list, sort_type: sort_type, page: page || 1 }, 'html').done(function (data) {
        selected = [];
