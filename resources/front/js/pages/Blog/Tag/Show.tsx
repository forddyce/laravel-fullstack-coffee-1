...
interface BlogTagShowProps extends PageProps {
    tag: BlogTag;
    blogs: {
        data: Blog[];
...
export default function BlogTagShow() {
    const { tag, blogs, filters } = usePage<BlogTagShowProps>().props;
    ...
    useEffect(() => {
        ...
        debounceTimeoutRef.current = window.setTimeout(() => {
            router.get(
                route('client.blog-tags.show', tag.slug),
    ...
    return (
        <ClientLayout>
            <Head title={`${tag.title} Blogs - WE Coffee Roasters Blog`} />

            <section className="bg-brand-primary py-16 text-center text-white">
                <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                    <h1 className="mb-4 text-5xl font-extrabold md:text-6xl">{tag.title}</h1>
                    <p className="text-lg leading-relaxed md:text-xl">Explore articles related to "{tag.title}".</p>
                </div>
            </section>
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="mb-8 flex flex-col items-center justify-between gap-4 md:flex-row">
                        <div className="flex flex-wrap justify-center gap-2 md:justify-start">
                            <Button onClick={() => router.get(route('client.blogs.index'))} variant={'secondary'} className="text-sm uppercase">
                                &larr; All Blogs
                            </Button>
                        ...
                        <div className="w-full md:w-1/3">
                            <TextInput
                                ...
                                placeholder={`Search in "${tag.title}"...`}
                    ...
                    {blogs.data.length > 0 ? (
                        ...
                        <p className="py-8 text-center text-gray-500">No blog posts found for this tag matching your criteria.</p>
                    )}
